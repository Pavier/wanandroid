 ...
 * @date:2021/9/6 16:51
 */
class TreeAdapter(data: MutableList<TreeData>) : BaseQuickAdapter<TreeData, BaseViewHolder>(R.layout.item_tree,data) {

    private var action : (data: TreeData,view : View,position : Int) -> Unit = {data,view,position ->}
    ...
    override fun convert(holder: BaseViewHolder, item: TreeData) {
        item.apply {
            ...
            recyclerView.run {
                ...
                adapter = TreeDetailAdapter(children as ArrayList<Children>).apply {
    ...
    fun setItemClick(action : (data : TreeData,view: View,position : Int) -> Unit){
        this.action = action
    }
}